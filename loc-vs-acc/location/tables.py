...
def compare_behav_types(data_file, min_sampels=2000, r=1, hard_max=3):
    ...
    for animal in animals:
        ...
        data = trajectory_processor(data, stamp=False).compute_first_passage(r, hard_max=hard_max).cluster("FPT_{}".format(r), k=3)
...
def marginals_etc(data_file, min_sampels=2000, r=1, hard_max=3):
    ...
    for animal in animals:
        ...
        data = trajectory_processor(data, stamp=False).compute_steps().compute_first_passage(r, hard_max=hard_max).cluster("FPT_{}".format(r), k=3)[:-1]
